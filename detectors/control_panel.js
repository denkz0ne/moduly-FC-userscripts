(function () {
    ...
    const DEFAULT_TOKENS = [
        ...
        'colorCode', 'folding', 'kind', 'canvas', 'giftPack', 'aliasKey', 'original', 'ext'
    ];
    ...
    function getPanelTop() {
        ...
        if (!topRow) return 94;
        return Math.max(8, Math.round(topRow.getBoundingClientRect().bottom + 8));
    }
    ...
    function ensureActionPanel() {
        ...
        Object.assign(panel.style, {
            position: 'fixed', right: '20px', top: `${getPanelTop()}px`, bottom: 'auto', zIndex: '900',
            width: '94px', display: 'flex', flexDirection: 'column', alignItems: 'stretch', gap: '5px'
        });
    ...
    function styleActionButton(button) {
        ...
        Object.assign(button.style, {
            position: 'static', right: 'auto', bottom: 'auto', display: 'block', width: '100%', minWidth: '0',
            boxSizing: 'border-box', margin: '0', padding: '8px 10px', textAlign: 'center', lineHeight: '13px',
            fontWeight: '700', color: '#fff', background: '#263238', border: '1px solid #111', borderRadius: '5px',
            cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.18)', textDecoration: 'none'
        });
    }
    ...
    function ensureButton() {
        ...
        if (!button) {
            ...
            button.title = 'MaterialDetector konfiguracia';
    ...
    function addStyles() {
        ...
        style.textContent = `
            #${PANEL_ID}{position:fixed;right:124px;top:94px;width:470px;max-width:calc(100vw - 150px);max-height:82vh;overflow:auto;z-index:901;background:#fff;color:#111;border:1px solid #222;border-radius:6px;box-shadow:0 8px 28px rgba(0,0,0,.32);font-family:Arial,sans-serif;font-size:12px;display:none}
            #${PANEL_ID} *{box-sizing:border-box} #${PANEL_ID} .mdcp-head{display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:#111;color:#fff;font-weight:700}
            #${PANEL_ID} .mdcp-body{padding:12px} #${PANEL_ID} label{display:block;font-weight:700;margin:9px 0 4px}
            #${PANEL_ID} select,#${PANEL_ID} input{width:100%;border:1px solid #bbb;border-radius:4px;padding:6px;font-size:12px}
            #${PANEL_ID} button{border:1px solid #333;border-radius:4px;background:#f4f4f4;padding:5px 7px;cursor:pointer;font-size:12px}
            #${PANEL_ID} button.primary{background:#111;color:#fff} #${PANEL_ID} button.danger{background:#b71c1c;color:#fff;border-color:#7f0000}
            #${PANEL_ID} .mdcp-row{display:flex;gap:6px;align-items:center;margin:5px 0} #${PANEL_ID} .mdcp-row>*{flex:1} #${PANEL_ID} .mdcp-row button{flex:0 0 auto}
            ...
            #${PANEL_ID} .mdcp-actions{display:flex;gap:7px;margin-top:12px;flex-wrap:wrap} #${PANEL_ID} .mdcp-small{color:#555;font-size:11px;line-height:1.35}
    ...
    function createBlockRow(block, tokens) {
        ...
        row.innerHTML = `<div class="mdcp-row"><select data-block-type><option value="token">token</option><option value="text">text</option></select><select data-block-token></select><input data-block-text type="text" placeholder="vlastny text"><button type="button" data-up>↑</button><button type="button" data-down>↓</button><button type="button" data-remove>x</button></div>`;
        ...
        function sync() { token.style.display = type.value === 'token' ? '' : 'none'; text.style.display = type.value === 'text' ? '' : 'none'; }
        ...
        row.addEventListener('drop', event => {
            ...
            const dragged = Array.from(row.parentNode.children)[from];
    ...
    function createTemplateEditor(field, label, template, tokens) {
        ...
        wrap.innerHTML = `<label>${esc(label)}</label><div data-blocks></div><div class="mdcp-row"><button type="button" data-add-token>+ token</button><button type="button" data-add-text>+ text</button><button type="button" data-clear>Vycistit</button></div>`;
        const list = wrap.querySelector('[data-blocks]');
        api.normalizeTemplateBlocks(template).forEach(block => list.appendChild(createBlockRow(block, tokens)));
    ...
    function renderPanel() {
        ...
        const tokens = getTokenList(detector);
        const state = window.__materialDetectorState || {};
        const aliasKey = (state.tokens && state.tokens.aliasKey) || (state.params && state.params.aliasKey) || '';
        const renameChecked = getCurrentRenameEnabled(settings) ? 'checked' : '';
        panel.style.top = `${getPanelTop()}px`;
        panel.innerHTML = `<div class="mdcp-head"><span>MaterialDetector CFG</span><button type="button" data-close>x</button></div><div class="mdcp-body"><label>Detector</label><select data-detector-select>${detectors.map(item => `<option value="${esc(item.id)}" ${item.id === detector.id ? 'selected' : ''}>${esc(item.displayName || item.id)}</option>`).join('')}</select><div class="mdcp-section"><label>Aktualny stav</label><div class="mdcp-preview">${esc(detectorPreview())}</div></div><div class="mdcp-section"><label>Dostupne tokeny</label><div>${tokens.map(t => `<span class="mdcp-token">{${esc(t)}}</span>`).join('')}</div><div class="mdcp-preview">${esc(tokenPreview())}</div></div><div data-editors></div><div class="mdcp-section"><label><input data-rename-enabled type="checkbox" style="width:auto;vertical-align:middle" ${renameChecked}> Zapnut renamer pre tento detector</label><div class="mdcp-small">Ulozenim checkbox explicitne nastavi renamer pre tento detector.</div></div><div class="mdcp-section"><label>Alias key</label><input data-alias-key value="${esc(aliasKey)}" placeholder="napr. 42foto/web|economy plagat|180g"><label>Alias value</label><input data-alias-value placeholder="napr. 180"><div class="mdcp-row"><button type="button" data-save-alias>Ulozit alias</button><button type="button" data-clear-alias>Vymazat alias</button></div></div><div class="mdcp-actions"><button class="primary" type="button" data-save>Ulozit konfiguraciu</button><button type="button" data-reset>Reset detectora</button><button class="danger" type="button" data-close>Zavriet</button></div></div>`;
    ...
    function saveCurrentSettings() {
        ...
        const detector = getSelectedDetector();
        if (!detector) return;
        api.saveDetectorSettings(detector.id, {
    ...
    function saveAlias() {
        const panel = ensurePanel();
        if (!api.setMaterialAlias(panel.querySelector('[data-alias-key]').value, panel.querySelector('[data-alias-value]').value)) return;
        window.dispatchEvent(new CustomEvent('materialDetector:configChanged'));
        onChange();
        renderPanel();
    }
    ...
    function clearAlias() {
        const panel = ensurePanel();
        if (!api.clearMaterialAlias(panel.querySelector('[data-alias-key]').value)) return;
        window.dispatchEvent(new CustomEvent('materialDetector:configChanged'));
        onChange();
        renderPanel();
    }

    function openPanel() { const panel = ensurePanel(); renderPanel(); panel.style.display = 'block'; }
    function closePanel() { ensurePanel().style.display = 'none'; }
    function togglePanel() { const panel = ensurePanel(); panel.style.display === 'block' ? closePanel() : openPanel(); }
