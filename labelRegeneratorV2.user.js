...
(function () {
    ...
    function normalizeBlockConfig(input, defaults) {
        const block = deepClone(defaults);
        const incomingContainer = input && input.container ? input.container : {};
        const incomingText = input && input.text ? input.text : {};

        Object.keys(block.container).forEach((key) => {
            block.container[key] = normalizeValue(key, incomingContainer[key], block.container[key], 'container');
        });

        Object.keys(block.text).forEach((key) => {
            block.text[key] = normalizeValue(key, incomingText[key], block.text[key], 'text');
        });

        sanitizeBlockConfig(block);
        return block;
    }

    function normalizeValue(key, value, fallback, scope) {
    ...
    function findZoneDefinition(zoneName) {
        const normalized = String(zoneName || '').trim();
        return ZONE_DEFINITIONS.find((definition) => definition.key === normalized || definition.aliases.includes(normalized)) || null;
    }
    ...
    function resolveZoneValue(definition) {
        ...
        for (const key of candidates) {
            const stored = getStoredZoneValue(key);
    ...
    function persistZoneValue(definition, value) {
        ...
        definition.aliases.forEach((alias) => {
            window[alias] = normalized;
            setStoredZoneValue(alias, normalized);
        });
        return normalized;
    }
    ...
    function injectBaseStyles() {
        ...
        style.textContent = `
            :root {
                ...
                --lr-panel-width: 360px;
                --lr-guide-color: rgba(25, 118, 210, 0.85);
                --lr-guide-fill: rgba(25, 118, 210, 0.08);
                --lr-panel-shadow: 0 18px 42px rgba(0, 0, 0, 0.18);
                --lr-panel-border: rgba(0, 0, 0, 0.08);
            }
            ...
            html.lr-edit-mode #label::after {
                ...
                position: absolute;
                inset: 0;
                border: 1px dashed var(--lr-guide-color);
            ...
            html.lr-edit-mode .lr-block {
                outline: 1px dashed var(--lr-guide-color);
                outline-offset: -1px;
                background-image: linear-gradient(180deg, var(--lr-guide-fill), transparent);
            }
            ...
            .lr-guide-tag {
                ...
                border-right: 1px solid rgba(13, 71, 161, 0.3);
                border-bottom: 1px solid rgba(13, 71, 161, 0.3);
                display: none;
                pointer-events: none;
            }

            html.lr-edit-mode .lr-guide-tag {
                display: block;
            }
            ...
            #lr-config-panel {
                ...
                border-radius: 18px;
                box-shadow: var(--lr-panel-shadow);
            ...
            .lr-panel-topbar select,
            .lr-block-form input,
            .lr-block-form select {
                width: 100%;
                box-sizing: border-box;
    ...
    function buildZone(definition) {
        ...
        guide.textContent = definition.title;
        zone.appendChild(guide);

        const anchor = document.createElement('div');
        anchor.className = `lr-text-anchor lr-text-anchor--${definition.slug}`;

        const text = document.createElement('span');
        text.className = `lr-text lr-text--${definition.slug}`;
    ...
    function ensureOverlayLayer() {
        ...
        ZONE_DEFINITIONS.forEach((definition) => {
            overlay.appendChild(buildZone(definition));
        });
    ...
    function getZoneElements(definition) {
        const zone = document.getElementById(definition.id);
        if (!zone) return null;
        return {
            zone,
            anchor: zone.querySelector('.lr-text-anchor'),
            text: zone.querySelector('.lr-text')
        };
    }

    function applyBlockContainerStyles(definition, elements, config) {
        const zone = elements.zone;
        const anchor = elements.anchor;
        const container = config.container;
        const text = config.text;

        zone.style.display = container.visible ? 'block' : 'none';
        ...
        anchor.style.justifyContent = text.textAlign === 'right'
            ? 'flex-end'
    ...
    function applyBlockTextStyles(elements, config, content) {
        ...
        text.style.transformOrigin = textConfig.textAlign === 'right'
            ? 'right center'
    ...
    function fitZoneText(elements, config) {
        const anchor = elements.anchor;
        const text = elements.text;
        const textConfig = config.text;

        text.style.transform = 'scale(1, 1)';
        const scaleX = textConfig.scaleX / 100;
        const naturalWidth = text.scrollWidth * scaleX;
        const naturalHeight = text.scrollHeight;
        const availableWidth = anchor.clientWidth;
        const availableHeight = anchor.clientHeight;

        if (availableWidth <= 0 || availableHeight <= 0 || naturalWidth <= 0 || naturalHeight <= 0) {
            text.style.transform = `scale(${scaleX}, 1)`;
            return;
        }

        const fitWidth = availableWidth / naturalWidth;
        const fitHeight = availableHeight / naturalHeight;
        const fitScale = Math.min(1, fitWidth, fitHeight);
        text.style.transform = `scale(${scaleX * fitScale}, ${fitScale})`;
    }
    ...
    function createInput(field, value, blockKey, scope) {
    ...
    function buildBlockCard(definition) {
        ...
        note.textContent = `Classy: .lr-block--${definition.slug} a .lr-text--${definition.slug}`;
        card.appendChild(note);

        const blockForm = document.createElement('div');
        blockForm.className = 'lr-block-form';

        const containerTitle = document.createElement('div');
        containerTitle.className = 'lr-section-title';
        containerTitle.textContent = 'Block';
        blockForm.appendChild(containerTitle);

        const containerGrid = document.createElement('div');
        containerGrid.className = 'lr-field-grid';
        FIELD_GROUPS.container.forEach((field) => {
            const configValue = layoutConfig.blocks[definition.key].container[field.key];
            containerGrid.appendChild(createInput(field, configValue, definition.key, 'container'));
        });
        blockForm.appendChild(containerGrid);

        const textTitle = document.createElement('div');
        textTitle.className = 'lr-section-title';
        textTitle.textContent = 'Text Element';
        blockForm.appendChild(textTitle);

        const textGrid = document.createElement('div');
        textGrid.className = 'lr-field-grid';
        FIELD_GROUPS.text.forEach((field) => {
            const configValue = layoutConfig.blocks[definition.key].text[field.key];
            textGrid.appendChild(createInput(field, configValue, definition.key, 'text'));
        });
        blockForm.appendChild(textGrid);

        card.appendChild(blockForm);
        return card;
    }
    ...
    function updateEditorInputValues() {
        ...
        panelElements.panel.querySelectorAll('input, select').forEach((input) => {
            const blockKey = input.dataset.block;
            const scope = input.dataset.scope;
            const field = input.dataset.field;
            if (!blockKey || !scope || !field) return;

            const value = layoutConfig.blocks[blockKey][scope][field];
    ...
    function handleConfigInput(event) {
        ...
        const blockKey = target.dataset.block;
        const scope = target.dataset.scope;
        const field = target.dataset.field;
        if (!blockKey || !scope || !field) return;

        const fieldDefinition = FIELD_GROUPS[scope].find((item) => item.key === field);
        ...
        layoutConfig.blocks[blockKey][scope][field] = value;
        sanitizeBlockConfig(layoutConfig.blocks[blockKey]);
        saveLayoutConfig();
        renderOverlayZones();
    ...
    function ensureConfigPanel() {
        ...
        viewButton.className = 'lr-btn lr-btn--primary';
        viewButton.textContent = 'View';
        topbar.appendChild(viewButton);

        const editButton = document.createElement('button');
        editButton.type = 'button';
        editButton.className = 'lr-btn';
        ...
        footer.className = 'lr-panel-footer';
        footer.textContent = 'Shortcut: Ctrl+Shift+E otvori alebo skryje editor.';
        ...
        resetButton.addEventListener('click', () => {
            layoutConfig = buildDefaultLayoutConfig();
            saveLayoutConfig();
            renderOverlayZones();
            updateEditorInputValues();
        });
    ...
    function exposeOverlayApi() {
        window.labelRegeneratorSetZone = (zoneName, value) => {
            const definition = findZoneDefinition(zoneName);
            if (!definition) return;

            persistZoneValue(definition, value);
            renderOverlayZones();
        };

        window.labelRegeneratorRefresh = renderOverlayZones;
        window.labelRegeneratorGetConfig = () => deepClone(layoutConfig);
        window.labelRegeneratorSetBlockConfig = (blockKey, patch) => {
            const definition = findZoneDefinition(blockKey);
            if (!definition || !patch || typeof patch !== 'object') return;

            const current = layoutConfig.blocks[definition.key];
            const next = normalizeBlockConfig(
                {
                    container: Object.assign({}, current.container, patch.container || {}),
                    text: Object.assign({}, current.text, patch.text || {})
                },
                ZONE_DEFINITIONS.find((item) => item.key === definition.key).defaults
            );

            layoutConfig.blocks[definition.key] = next;
            saveLayoutConfig();
            renderOverlayZones();
            updateEditorInputValues();
        };
    ...
    function syncLabelValuesFromDetailPage() {
        ...
        const leftValue = leftBadge ? (leftBadge.textContent || '').trim() : getStoredZoneValue('TM_testoLeft');
        const rightValue = rightBadge ? (rightBadge.textContent || '').trim() : getStoredZoneValue('TM_testoRight');

        persistZoneValue(findZoneDefinition('TM_testoLeft'), leftValue);
        persistZoneValue(findZoneDefinition('TM_testoRight'), rightValue);
    }
    ...
    function pressLAction() {
        ...
        popup.addEventListener('load', () => {
            triggerPrintWhenReady(popup);
        }, { once: true });
    }

    function initializeWindowValues() {
        ZONE_DEFINITIONS.forEach((definition) => {
            const value = resolveZoneValue(definition);
            window[definition.key] = value;
            definition.aliases.forEach((alias) => {
                window[alias] = value;
            });
        });
    ...
    function bindEvents() {
        ...
        window.addEventListener('unload', () => {
            if (!isPrintLabelPage()) return;
            const leftDefinition = findZoneDefinition('TM_testoLeft');
            const rightDefinition = findZoneDefinition('TM_testoRight');
            [leftDefinition, rightDefinition].forEach((definition) => {
                if (!definition) return;
                localStorage.removeItem(definition.key);
                definition.aliases.forEach((alias) => localStorage.removeItem(alias));
            });
        });
    }
